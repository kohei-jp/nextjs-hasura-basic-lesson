...
// todoVarに追加していくコンポーネント
export const LocalStateA: VFC = () => {
  const [input, setInput] = useState('');
  const todos = useReactiveVar(todoVar);  // makeVarにアクセス.

  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault() // submitBtnは、pageがリロードするので防止.  
    // todoVar: makeVarのstateに値を入れている.
  ...
  return (
    <>
      ...
      >
        ...
        />
        <button
          disabled={!input} // inputが空の場合は押せなくする.
